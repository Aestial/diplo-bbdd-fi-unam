...
#d01
if [ -d "/unam/diplo-bd/disks/d01/app/oracle/oradata/${ORACLE_SID^^}" ]; then
  echo "Directorio para data files ya existe, se omite creación"
else
  echo "1. Creando directorios para Redo Logs y control files en d01"  
  cd /unam/diplo-bd/disks/d01
...
#d02
if [ -d "/unam/diplo-bd/disks/d02/app/oracle/oradata/${ORACLE_SID^^}" ]; then
  echo "Directorio para data files ya existe, se omite creación"
else
  echo "1. Creando directorios para Redo Logs y control files en d02"
  cd /unam/diplo-bd/disks/d02
  mkdir -p app/oracle/oradata/${ORACLE_SID^^}
  chown -R oracle:oinstall app
  chmod -R 750 app  
fi;
...
#d03
if [ -d "/unam/diplo-bd/disks/d03/app/oracle/oradata/${ORACLE_SID^^}" ]; then
  echo "Directorio para data files ya existe, se omite creación"
else
  echo "1. Creando directorios para Redo Logs y control files en d03"
  cd /unam/diplo-bd/disks/d03
  mkdir -p app/oracle/oradata/${ORACLE_SID^^}
  chown -R oracle:oinstall app
  chmod -R 750 app  
fi;
...
#d01
cd  /unam/diplo-bd/disks/d01/app/oracle/oradata/${ORACLE_SID^^}
rm -f *.log *.ctl
#d02
cd  /unam/diplo-bd/disks/d02/app/oracle/oradata/${ORACLE_SID^^}
rm -f *.log *.ctl
#d03
cd  /unam/diplo-bd/disks/d03/app/oracle/oradata/${ORACLE_SID^^}
rm -f *.log *.ctl
